...
def process_soils(source_da, nhru_gdf, output_path, source_type, vpu, categorical, id_feature="hru_id"):
    """
...
def process_soil_moist_max(base_source_dir, nhru_gdf, output_path, source_type, vpu, categorical, id_feature="hru_id"):
    """
    ...
    final_rast = base_source_dir / "lulc_veg/rd_250_raw.tif"  # assuming this is the intended final raster
    ...
    if not soil_moist_max_rast.exists():
        raise FileNotFoundError(f"Soil_moist_max raster not found: {soil_moist_max_rast}")
    
    print(f"Loaded soil_moist_max raster: shape={source_da.shape}, crs={source_da.rio.crs}")
...
def main():
    parser = argparse.ArgumentParser(description="Create soils parameters from source raster data.")
    parser.add_argument("--config", required=True, help="Path to config YAML file")
    parser.add_argument("--vpu", required=True, help="VPU code, e.g., 01, 03N, 03S, 03W, 10U, 10L")
    ...
    if source_type == "soils":
        ...
        process_soils(source_da, nhru_gdf, output_path, source_type, args.vpu, categorical)
        
    elif source_type == "soil_moist_max":
        print("Processing soil_moist_max data...")
        process_soil_moist_max(base_source_dir, nhru_gdf, output_path, source_type, args.vpu, categorical)
    else:
