...
def test_basic_ratio_no_clamp():
    ...
    out = compute_ratio(num, den, "nat_hru_id", "count", "p", clamp_to_one=False)
...
def test_clamp_to_one_applied():
    ...
    out = compute_ratio(num, den, "nat_hru_id", "count", "carea", clamp_to_one=True)
...
def test_output_columns_are_id_and_param_only():
    num = _df([1], [5.0])
    den = _df([1], [10.0])
    out = compute_ratio(num, den, "nat_hru_id", "count", "my_param", clamp_to_one=False)
