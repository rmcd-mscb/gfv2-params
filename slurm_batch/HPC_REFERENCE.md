...
| (run directly) | `configs/base_config.yml` | `gfv2_params.download.nhd_topology` — downloads per-VPU NHDPlusV2 `PlusFlowlineVAA` attributes and distills COMID/DnHydroseq/Hydroseq/TerminalFl/StartFlag/StreamOrde to `input/nhd/flowline_topology.parquet` (one-time, CONUS); must run before `stage_nhd_flowthrough.batch` below |
