...
2. `gfv2_dev` for the CONUS shakedown, **not** `gfv2` (standing rule: validate
   unproven rebuilds on `gfv2_dev`). `gfv2_dev`'s `waterbody_gpkg` fix is a
   prerequisite for this step to mean anything. Expect `segment_wbody` to emit
   48,529 COMIDs; a materially different count means an input changed.
