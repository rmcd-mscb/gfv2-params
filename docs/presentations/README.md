...
- `2026-07-depression-storage-workflow.slides.md` — a **technical design
  review** of the PRMS/NHM depression-storage parameter workflow, framed
  rule-first: each on-stream / dprst classification rule stated as a rule and
  shown firing on named hydrography, from the legacy ArcPy pipeline
  (`docs/0b_TB_depr_stor.py`) through the endorheic terminus-inside-itself
  classifier in **PR #178, which is unmerged**. Method/workflow-focused; spans
  fabrics. References figures under `../figures/depstor/`, generated by
  `scripts/render_depstor_figures.py` — **must be run under `srun`**, not on
  the login node, since it reads CONUS-scale rasters.
