...
│   ├── 3dep/                   # dem_1m_tile_inventory.parquet — the real, staged
│                                #   3DEP 1 m tile inventory (issue #223 part 2); a
│                                #   SNAPSHOT, fabric-independent, shared by every
│                                #   fabric's dprst_depth step
│   ├── wesm/                   # wesm_project_attrs.parquet — WESM read geometry-free
│                                #   (quality/date only), staged alongside 3dep/
...
| `vpu` | — | ✓ | The one VPU this fabric lies in, as a string (`"12"`, `"14"`, `"17"`). `vpu_id` fills a constant raster with it and `carea_map`'s percentile mode looks up that VPU's TWI thresholds (`twi_reference_percentiles.hydrodem.csv`). Resolution order (`depstor_builders/vpu_id.py::resolve_vpu_source`): profile scalar, else a per-HRU `vpu` attribute on the HRU layer, else `vpu_id` **raises**. Omit it only on a multi-VPU fabric whose `nhru` layer carries a `vpu` column (gfv2). Sub-region labels (`03N`, `10U`) map to their parent raster VPU. Every single-gpkg regional fabric sets it; only the multi-VPU `gfv2` family omits it. |
| `dem_1m_inventory` | — | ✓ | Path to `input/3dep/dem_1m_tile_inventory.parquet` — the real, staged inventory of every published USGS 3DEP 1 m tile, with its actual (often cropped) extent read from its own COG header (`gfv2_params.dprst_depth.inventory.build_inventory`). Staged by `pixi run python -m gfv2_params.download.dem_1m_inventory` / `slurm_batch/stage_dem_1m_inventory.batch` (issue #223 part 2; replaces the retired convex-hull `wesm_index`). Consumed by `dprst_depth`'s `sources.tag_and_assign` (the single entry point shared by the in-process builder and the SLURM planner) to rank each polygon's candidate tile sets; required for `dprst_depth`, not for any other depstor step. A SNAPSHOT of what USGS publishes — re-staging obliges a `dprst_depth` re-run of every fabric, the same obligation a `shared_rasters` rebuild carries. |
| `wesm_project_attrs` | — | ✓ | Path to `input/wesm/wesm_project_attrs.parquet` — WESM read **geometry-free** (`gfv2_params.dprst_depth.inventory.project_attrs`), keyed by S3 project directory, carrying only quality (`ql`) and collection-date (`collect_end`) attributes. Staged alongside `dem_1m_inventory` by the same command/batch. `sources.tag_and_assign` joins it to rank candidate tile sets by quality then newest collection date; WESM's geometry (the convex-hull footprint that motivated this rewrite) is never read. |
| `min_dem_1m_tiles` / `min_dem_1m_projects` / `min_wesm_project_attrs_rows` | — | — | Integer floors on `dem_1m_inventory`'s tile count / distinct-project count, and on `wesm_project_attrs`'s row count, enforced in `sources.tag_and_assign` (the same shared entry point `dem_1m_inventory` above is consumed by) — same both-ends/consuming-end doctrine as `min_onstream_comids`. An empty inventory or attrs table always raises unconditionally; below the floor otherwise raises too. Module-constant defaults (`DEFAULT_MIN_INVENTORY_TILES=100_000`/`DEFAULT_MIN_INVENTORY_PROJECTS=900`/`DEFAULT_MIN_PROJECT_ATTRS_ROWS=500` in `sources.py`, measured against the full CONUS inventory: 125,627 tiles/939 projects, `wesm_project_attrs` 932 rows) apply unless overridden here; 0 or negative disables that one check. No fabric profile sets these today — every profile points at the same shared, CONUS-wide staged file, so there is no legitimate reason to lower them. |
...
- **CONUS-scale COMPUTE (not memory): `dprst_depth` is per-polygon, not
  ...
  assigned tile SET (`tiling.tile_set_groups`/`tile_batches`, keyed on the
  ranked candidates `sources.tag_and_assign` assigns from the staged 3DEP
  tile inventory, issue #223 part 2), not HRU
...
that job, keyed on the assigned tile SET rather than the HRU batch every other
array in this repo uses (`src/gfv2_params/dprst_depth/tiling.py`'s
`tile_set_groups`/`tile_batches`, `scripts/run_dprst_depth_batch.py`,
